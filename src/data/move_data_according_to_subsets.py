...
def move_data(raw_data_path, processed_data_path):
    ...
    for whichSplit in dataSplit:
        ...
        imgNames = pd.read_csv(raw_data_path + '/' + whichSplit + '.txt',
        ...
        for imgName in imgNames['file_name']:
            ...
            if os.path.isfile(tilesFolder + '/' + image) \
                    and not os.path.isfile(moveToSubset + '/images/img/' + image):
                shutil.move(tilesFolder + '/' + image,
                            moveToSubset + '/images/img/' + image)

            if os.path.isfile(tilesFolder + '/' + org_mask_name) \
                    and not os.path.isfile(moveToSubset + '/masks/img/' + mask):
                shutil.move(tilesFolder + '/' + org_mask_name,
